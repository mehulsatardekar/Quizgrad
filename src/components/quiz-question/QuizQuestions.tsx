...
const QuizQuestions = ({Question, Totalquestions, Currentquestion, SetAnswer}: quizQuestionPropsType) => {
  ...
  if(progressBar.current !== null){
   ...
   return ()=> {gotoNextQuestion; clearTimeout(timer.current) } 
  },[Question])
    ...
    <section >
      {
        Question && (
          ...
          <div className="flex flex-column gap-1">
            ...
            <span className="font-md-light">Question {Currentquestion}/{Totalquestions}</span>
          </div>
          <div>
      ...
      <section className="flex  flex-justify-center flex-align-item-center">
        ...
        <div className="ui-card-text card-mb-1 ">
          <section className="flex flex-justify-center width-full ">
              <div className="flex flex-column gap-2 pt-1 ">
                  <span className="font-bold font-weight-800 font-sm line-space-2">{Question.question}</span>
                   {
                     Question.option.map((option:string, index:number)=>{
                       return(
                    <label htmlFor={`ids_ ${index}`} className="btn-outline-primary btn btn-py-1 flex flex-justify-center gap">{option}
                    <input type='radio'
                        className={`option-radio ${index == optionSelected? 'option active': 'option' }`}
                        key={index+"_keys"}
                        id={`ids_ ${index}`}
                        name="ts"
                        value={option}
        ...
        <div className="flex flex-justify-center flex-align-item-center mt-2 mb-1">
            <button  className="btn-dark btn btn-py-1 flex flex-justify-center flex-align-item-center"
