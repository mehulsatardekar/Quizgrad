          ...
          userOpt = useranswers[i];
            return(
               ...
               <div className="flex flex-between flex-align-item-center">
                 ...
                 <span className="label-text label-text-primary font-sm">Question {i+1}</span>
                 <span className="label-text ">Quiz Marks {quiz.points}</span>
                 </div>
                 <span className="label-text label-text-success">You Scored:{(userOpt === quiz.answerIndex)?quiz.points: 0}</span>
               </div>
               ...
               quiz.option.map((option, optionIndex)=>{
                   return(
                      <span key={`${optionIndex}-${id}`} className={`btn btn-py-1 flex flex-justify-center disable-btn disable 
                       ${(optionIndex === userOpt && userOpt === quiz.answerIndex)? 'btn-success': 
                        (optionIndex === userOpt && userOpt !== quiz.answerIndex)?('btn-danger'):
                        (optionIndex == quiz.answerIndex)? 'btn-success':
